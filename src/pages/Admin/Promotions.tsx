...
export default function Promotions() {
  ...
  const classNames = useMemo(() => {
    ...
    students.forEach((student) => {
      if (student.class) names.add(student.class);
    });
  ...
  const classOptions = useMemo(
    () =>
      classNames.map((name) => ({
        value: name,
        label: name,
        sublabel: `${students.filter((s) => s.class === name).length} student(s)`,
      })),
    [classNames, students],
  );
  ...
  const targetOptions = useMemo(
    () => [
      ...classOptions.filter((option) => option.value !== sourceClass),
      { value: GRADUATE_VALUE, label: 'Graduated', sublabel: 'Complete programme & mark as graduated' },
    ],
    [classOptions, sourceClass],
  );

  const suggestedTarget = useMemo(() => {
    const index = classNames.indexOf(sourceClass);
    if (index === -1) return '';
    return classNames[index + 1] ?? GRADUATE_VALUE;
  }, [classNames, sourceClass]);

  const candidates = useMemo(() => {
    if (!sourceClass) return [];
    return students
      .filter((student) => student.class === sourceClass)
      .filter((student) => includeInactive || student.status === 'Active')
      .sort((a, b) => a.name.localeCompare(b.name));
  }, [students, sourceClass, includeInactive]);
  ...
  const handleSourceChange = (value: string) => {
    ...
    setTargetClass(index === -1 ? '' : classNames[index + 1] ?? GRADUATE_VALUE);
  };

  const handlePromote = () => {
    if (!sourceClass || !targetClass || selectedIds.length === 0) return;
    const isGraduating = targetClass === GRADUATE_VALUE;
    const moved = isGraduating ? bulkGraduateStudents(selectedIds) : bulkUpdateStudentClass(selectedIds, targetClass);
      ...
      description:
        moved > 0
          ? `${moved} student${moved === 1 ? '' : 's'} ${
              isGraduating ? 'marked as graduated' : `moved from ${sourceClass} to ${targetClass}`
            }.`
  ...
  const targetLabel = targetClass === GRADUATE_VALUE ? 'Graduated' : targetClass;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Student Promotion</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Move learners up to the next class or graduate them when they complete the programme.
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <AnimatedCard className="p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900 dark:text-white">Move from</p>
              <p className="text-xs text-slate-400">Select the current class</p>
            </div>
          </div>
          <SearchableSelect
          ...
          {sourceClass && (
            <p className="mt-3 text-xs font-medium text-slate-500 dark:text-slate-400">
              {candidates.length} eligible student{candidates.length === 1 ? '' : 's'} in {sourceClass}
            </p>
          )}
        </AnimatedCard>
        ...
        <AnimatedCard className="p-5" delay={0.05}>
          <div className="flex items-center justify-between gap-2 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900 dark:text-white">Move to</p>
                <p className="text-xs text-slate-400">Select the destination</p>
              </div>
            </div>
            {suggestedTarget && sourceClass && (
              <button
                type="button"
                onClick={() => setTargetClass(suggestedTarget)}
      ...
      <AnimatedCard className="p-5" delay={0.1}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-bold text-slate-900 dark:text-white">Select students</p>
            <p className="text-xs text-slate-400">
              {sourceClass ? `${selectedIds.length} of ${candidates.length} selected` : 'Choose a source class first'}
            </p>
          </div>
        ...
        <div className="relative mt-4">
          ...
          <input
            ...
            placeholder="Search by name, reg no or admission no..."
        ...
        <div className="mt-4 max-h-[26rem] overflow-y-auto rounded-2xl border border-slate-100 dark:border-slate-700/60">
          {!sourceClass ? (
            <p className="px-4 py-10 text-center text-sm font-medium text-slate-400">
              Select a source class to see the students.
            </p>
      ...
      <AnimatedCard className="p-5" delay={0.15}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="text-sm">
              {sourceClass && targetClass ? (
                <p className="font-semibold text-slate-800 dark:text-slate-100">
                  Move <span className="text-blue-600 dark:text-blue-300">{selectedIds.length}</span> student
                  {selectedIds.length === 1 ? '' : 's'} from{' '}
                  <span className="text-slate-500 dark:text-slate-400">{sourceClass}</span> to{' '}
                  <span className="text-slate-500 dark:text-slate-400">{targetLabel}</span>
                </p>
              ) : (
                <p className="font-medium text-slate-400">Choose a source and destination class to continue.</p>
              )}
            </div>
          </div>
          <button
            type="button"
            disabled={!sourceClass || !targetClass || selectedIds.length === 0}
          ...
          >
            {targetClass === GRADUATE_VALUE ? <GraduationCap className="h-4 w-4" /> : <UserCheck className="h-4 w-4" />}
            {targetClass === GRADUATE_VALUE ? 'Graduate selected' : 'Promote selected'}
          </button>
        </div>
      </AnimatedCard>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  You are about to {targetClass === GRADUATE_VALUE ? 'mark' : 'move'}{' '}
                  <span className="font-semibold text-slate-700 dark:text-slate-200">{selectedIds.length}</span> student
                  {selectedIds.length === 1 ? '' : 's'} from{' '}
                  <span className="font-semibold text-slate-700 dark:text-slate-200">{sourceClass}</span> to{' '}
                  <span className="font-semibold text-slate-700 dark:text-slate-200">{targetLabel}</span>. This updates
