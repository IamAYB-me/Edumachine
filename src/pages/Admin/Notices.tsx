...
import { useDataStore, Notice } from '@/store/useDataStore';
...
export default function AdminNoticeBoard() {
  ...
  const handleOpenModal = (notice?: Notice) => {
    if (notice) {
      ...
      setFormData({ title: notice.title, content: notice.content, targetAudience: notice.targetAudience, priority: notice.priority, author: notice.author });
    } else {
      setEditingNotice(null);
      setFormData({ title: '', content: '', targetAudience: 'All', priority: 'Medium', author: user?.name || 'Admin' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingNotice) {
      ...
      showToast({ title: 'Notice updated', variant: 'success' });
    } else {
      addNotice({
        ...(formData as Omit<Notice, 'id'>),
        date: new Date().toISOString().split('T')[0],
      });
      showToast({ title: 'Notice posted', variant: 'success' });
    }
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this notice?')) {
      deleteNotice(id);
      showToast({ title: 'Notice deleted', variant: 'info' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Total Notices" value={stats.total.toString()} icon={Bell} iconBgClass="bg-blue-50 dark:bg-blue-900/20" iconColorClass="text-blue-600 dark:text-blue-400" />
        <KPICard title="High Priority" value={stats.highPriority.toString()} icon={Megaphone} iconBgClass="bg-rose-50 dark:bg-rose-900/20" iconColorClass="text-rose-600 dark:text-rose-400" />
        <KPICard title="Posted Today" value={stats.today.toString()} icon={Calendar} iconBgClass="bg-emerald-50 dark:bg-emerald-900/20" iconColorClass="text-emerald-600 dark:text-emerald-400" />
        <KPICard title="Target Groups" value={stats.audience.toString()} icon={User} iconBgClass="bg-indigo-50 dark:bg-indigo-900/20" iconColorClass="text-indigo-600 dark:text-indigo-400" />
      </div>
        ...
        <div className="p-6 grid grid-cols-1 gap-4">
            ...
            <div key={notice.id} className="bg-white dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 hover:border-blue-200 dark:hover:border-blue-900/50 transition-all group">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  ...
                  )}><Megaphone className="w-5 h-5" /></div>
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-1">
                      <span className="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider"><Calendar className="w-3 h-3" /> {notice.date}</span>
                      <span className="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider"><User className="w-3 h-3" /> {notice.author}</span>
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300">For: {notice.targetAudience}</span>
                    </div>
                  </div>
                </div>
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => handleDelete(notice.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed pl-12">{notice.content}</p>
            </div>
          ))}
          {filteredNotices.length === 0 && (
            <div className="py-20 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600">
              ...
              <p className="text-xs text-slate-400 mt-1">Create your first notice to broadcast to the school community.</p>
            </div>
          )}
        </div>
      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <select value={formData.targetAudience} onChange={(e) => setFormData({ ...formData, targetAudience: e.target.value as Notice['targetAudience'] })}
                      className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-blue-500 dark:text-white">
                      <option value="All">All</option>
                      <option value="Students">{labels.learnerPlural}</option>
                      <option value="Teachers">{labels.teacherPlural}</option>
                      <option value="Parents">Parents</option>
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase">Priority</label>
                    <select value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: e.target.value as Notice['priority'] })}
                      className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-blue-500 dark:text-white">
