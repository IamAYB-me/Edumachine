 ...
 */
exports.payAcceptanceFee = functions.https.onCall(async (request) => {
    const data = request.data || {};
    const { reference, applicationFormNumber } = data;
    if (!reference || !applicationFormNumber) {
        throw new functions.https.HttpsError("invalid-argument", "Missing reference or applicationFormNumber.");
    ...
    try {
        ...
        if (!status || !txnData || txnData.status !== "success") {
            throw new functions.https.HttpsError("failed-precondition", `Payment verification failed: ${(txnData === null || txnData === void 0 ? void 0 : txnData.gateway_response) || "invalid transaction reference"}`);
        }
        ...
        const acceptanceStructure = activeAcceptance.find((s) => s.isUniversal === true) || activeAcceptance[0];
        ...
        if (acceptanceStructure && expectedAmount > 0 && Math.abs(paidAmount - expectedAmount) > 1) {
            throw new functions.https.HttpsError("failed-precondition", `Payment amount mismatch: expected ${expectedAmount}, received ${paidAmount}.`);
        }
        ...
        await db.collection("feeRecords").doc(feeRecordId).set({
            ...
            type: (acceptanceStructure === null || acceptanceStructure === void 0 ? void 0 : acceptanceStructure.category) || "Acceptance Fee",
            attachmentName: "",
            attachmentUrl: "",
            paymentReference: reference,
        ...
        if (uid) {
            ...
            await usersDocRef.set({ role: "STUDENT", roleLabel: "Student" }, { merge: true });
            const studentsDoc = db.collection("students").doc(uid);
            const existingStudent = await studentsDoc.get();
            if (!existingStudent.exists) {
                await studentsDoc.set({
                    id: uid,
                    name: fullName || (usersDoc.exists ? usersDoc.data().name : "") || email,
                    surname: app.surname || "",
                    ...
                    parentName: "",
                    status: "Active",
                    portalLevel: (usersDoc.exists ? usersDoc.data().portalLevel : "") || "College",
                });
            }
        }
        ...
        throw new functions.https.HttpsError("internal", "Acceptance fee verification failed. Please contact support.");
    }
});
/**
...
exports.deleteUserAccount = functions.https.onCall(async (request) => {
    ...
    for (const colName of ["teachers", "parents", "staff"]) {
        try {
            ...
            errors.push(`${colName}: ${(e === null || e === void 0 ? void 0 : e.message) || e}`);
        }
    }
    ...
    try {
        ...
        if (ids.size) {
            const feeSnap = await db
                ...
                .get();
            await Promise.all(feeSnap.docs.map((d) => d.ref.delete().then(() => results.push(`feeRecords/${d.id}`))));
        }
    }
    catch (e) {
        errors.push(`feeRecords: ${(e === null || e === void 0 ? void 0 : e.message) || e}`);
    }
